@php($title = 'Edit Pesanan Makanan')
@extends('layouts.app')
@section('content')
<div class="card">
  <div class="card-title" style="margin-bottom:12px;">Edit Pesanan</div>
  <form method="POST" action="{{ route('pesanan-makanan.update', $row->id ?? 0) }}">@csrf @method('PUT')
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
      ...
      <div>
        ...
        <select name="merchant_id" style="width:100%;padding:10px;border:1px solid #e5e7eb;border-radius:8px">
          ...
          <option value="{{ $m->id }}" @selected(($row->merchant_id ?? null) == $m->id)>{{ $m->nama_toko }}</option>
          @endforeach
        </select>
      </div>
      <div>
      ...
      <div><label>Total Bayar</label><input name="total_bayar" type="number" step="any" value="{{ $row->total_bayar ?? '' }}" style="width:100%;padding:10px;border:1px solid #e5e7eb;border-radius:8px"></div>
    </div>
