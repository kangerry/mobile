@php($title = 'Edit Tarif Delivery Toko')
@extends('layouts.app')
@section('content')
<div class="card">
  <div class="card-title" style="margin-bottom:12px;">Edit Tarif Delivery</div>
  <form method="POST" action="{{ route('tarif-delivery-toko.update', $item->id) }}">@csrf @method('PUT')
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;">
      ...
      <div>
        ...
        <select name="merchant_id" style="width:100%;padding:10px;border:1px solid #e5e7eb;border-radius:8px">
          <option value="" @if(!$item->merchant_id) selected @endif>Default Koperasi</option>
          @foreach($merchants as $m)
          <option value="{{ $m->id }}" @if($item->merchant_id==$m->id) selected @endif>{{ $m->nama_toko }}</option>
          @endforeach
        </select>
      </div>
